...
function create_recentcomments($args = '', $start = 0) {
	...
	if ($comments) {
		...
		if ($has_newer) {
			...
			$newer = '<div id="rc_newer"><a rel="nofollow" href="javascript:void(0);" onclick="RCJS.page(\'' . get_bloginfo('siteurl') . '\',\'' . $argsf . '\',' . $newer_start . ',\'' . $loading_text . '\');">' . __('Последние', 'wp-recentcomments') . '</a></div>';
		}

		// Older 按钮
		$older = '';
		if ($has_older) {
			$older = '<div id="rc_older"><a rel="nofollow" href="javascript:void(0);" onclick="RCJS.page(\'' . get_bloginfo('siteurl') . '\',\'' . $argsf . '\',' . $older_start . ',\'' . $loading_text . '\');">' . __('Предыдущие', 'wp-recentcomments') . '</a></div>';
		}
...
function rc_substring($str, $start=0, $len=50, $comment_id, $argsf, $comment_start, $loading_text, $code='UTF-8') {
	...
	if($code == 'UTF-8') {
		...
		$expand = ' <span class="rc_expand"><a rel="nofollow" href="javascript:void(0);" onclick="RCJS.detail(' . $comment_id . ',\'' . get_bloginfo('siteurl') . '\',\'' . $argsf . '\',' . $comment_start . ',\'' . $loading_text . '\');">' . __('Подробнее...', 'wp-recentcomments') . '</a></span>';
		return $str . $expand;
	}
}
