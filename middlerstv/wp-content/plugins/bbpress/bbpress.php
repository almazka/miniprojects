...
final class bbPress {
	...
	public static function instance() {
		...
		if ( null === $instance ) { // если инстанс нулевой (по-умолчанию он нулл), то создаем класс bbPress и его экземпляр кидаем в переменную $instance
			$instance = new bbPress;
			$instance->setup_globals(); // вызываем функцию новоиспеченого объекта setup_globals, она ничего не возвращает, просто принимает много разных значений разных переменных по теме версии и описания самого плагина
			$instance->includes(); // вызываем функцию новоиспеченого объекта includes, она ничего не возвращает, просто принимает много разных значений разных переменных по теме включений, пути адреса для файлов и т.д.
			$instance->setup_actions(); // то же самое по экшенам (в них не очень поняла), присваиваем значения многим переменным
		}

		// Always return the instance
		return $instance; // возвращаем объект bbPress, по сингл-тону, создается он один раз, а подключаться к нему могут все
	}
 ...
 */
function bbpress() {
	return bbpress::instance(); // перенаправляет на инстанс, а инстанс содержит в себе класс ключевой, и из него можно вытянуть разных кучу переменных
}
